...
const AddArtical = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to add artical');
        }
        setSuccess('Artical added successfully!');
        resetForm();
        setImageUrl('');
      } catch (err) {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white dark:bg-gray-900 rounded shadow border border-gray-200 dark:border-gray-700">
      <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Add Artical</h2>
      <form onSubmit={formik.handleSubmit}>
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? 'Submitting...' : 'Add Artical'}
        </button>
      </form>
    </div>
  );
