    ...
    <div className="max-w-3xl mx-auto p-6 bg-black/70 rounded-xl shadow-lg mt-8">
      ...
      {articals.length === 0 ? (
        ...
        <ul className="space-y-6">
          {articals.map((item) => (
            <li key={item._id} className="border border-gray-700 rounded-lg p-4 shadow hover:shadow-2xl transition bg-neutral-900/80 hover:bg-neutral-800/90 overflow-hidden max-h-[500px]">
              <h2 className="text-xl font-semibold mb-2 text-white break-words">{item.title}</h2>
              {item.image && (
                <img
                  src={item.image}
                  alt={item.title}
              ...
              <p className="text-gray-300 mb-2 break-words overflow-auto max-h-40">{item.content}</p>
